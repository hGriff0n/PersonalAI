...
pub struct Dispatcher {
    ...
    async_handles: sync::Arc<
        sync::RwLock<
            collections::HashMap<
                String, sync::Arc<Box<types::AsyncFunction<protocol::JsonProtocol>>>>>>,
}
...
impl Dispatcher {
    pub fn new() -> Self {
        Self{
            ...
            async_handles: sync::Arc::new(sync::RwLock::new(collections::HashMap::new())),
        }
    }
    ...
    pub fn async_dispatch(&self, mut rpc_call: types::Message, caller: net::SocketAddr)
        -> impl futures::Future<Item=types::Message, Error=()>
    {
        // Extract the handle from the handles map, making sure we release the RWLock before calling it
        // This is necessary for `register_app`, etc. as they require write access to the handles map
        // TODO: This will eventually just be the `self.handles`
        let handle = self.async_handles
            .read()
            .unwrap()
            .get(&rpc_call.call)
            .and_then(|handle| Some(handle.clone()));
