...
type RankingFunction = Fn(Vec<idx::Results>) -> idx::Results;
pub fn search(query: &str, index: &idx::Index, page_rank: &RankingFunction) -> idx::Results {
    let results = index.retrieve(query);
    page_rank(results)
}

pub fn intersect_rank(results: Vec<idx::Results>) -> idx::Results {
